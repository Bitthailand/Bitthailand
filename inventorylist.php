                            ...
                            <div class="table-responsive">
                                <table class="table table-hover text-nowrap table-sm">
                                    ...
                                    <tbody id="myTable">
                                        ...
                                        $result_count = mysqli_query($conn, "SELECT COUNT(*) As total_records FROM product  where  status='0' AND  ptype_id<>'TF0' $columx $keywordx  ");
                                        $total_records = mysqli_fetch_array($result_count);
                                        $total_records = $total_records['total_records'];
                                        $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                        $second_last = $total_no_of_pages - 1; // total page minus 1
                                        $result = mysqli_query($conn, "SELECT * FROM product where status='0' AND ptype_id <> 'TF0' $columx $keywordx LIMIT $offset, $total_records_per_page");
