...
if ($action == 'add_product') {
    ...
    VALUES ('$Forder_id','$Fcus_id','$Fcus_back','$Fcus_type_id','$emp_id','0')";
        ...
        $sql = "INSERT INTO order_details (order_id,ptype_id,product_id,qty,unit_price,total_price,status_button,emp_id,status_chk_stock,qty_out,updates,status_new)
        VALUES ('$Forder_id','$Fproduct_type','$row5[product_id]','$send_qty','$send_price','$send_total','1','$emp_id','TF','$send_price','1','1')";
        ...
        if (mysqli_num_rows($result) > 0) { ?>
            ...
            if ($Fcus_back == 1) {
                ...
                if ($chk1 == 'true' && $chk2 = 'true') {
                    ...
                    VALUES ('$Forder_id','$Fproduct_type','$Fproductx','$sum_qty','$Funit_price','$total_price','1','$emp_id','CB2','$Fqty','$Fqty2','$sum_qty','1','1')";
                    if ($conn->query($sqlx4) === TRUE) { ?>
                        <script>
                            $(document).ready(function() {
                                showAlert("บันทึกข้อมูลสำเร็จปปปปป", "alert-success");
                            });
                        </script>
                    <?php   } //
                ...
                if ($chk1 == 'true' && $chk2 = 'true') {
                   ...
                   $cus_stock1=$row5['fac1_stock']-$Fqty;
                   $cus_stock2=$row5['fac2_stock']-$Fqty2;
                   $sum_unit=$row5['unit_price']-$disunit;
                   $sum_total=$sum_unit*$sum_qty;
                   $sql11 = "UPDATE product  SET fac1_stock='$cus_stock1',fac2_stock='$cus_stock2' where product_id='$Fproductx' ";
                   if ($conn->query($sql11) === TRUE) {
                   }

                   $sqlx = "INSERT INTO deliver_detail (dev_id,product_id,order_id,dev_qty,unit_price,total_price,disunit,ptype_id,status_new,cus_back)
                   VALUES ('$dev_id','$Fproductx','$Forder_id','$sum_qty','$row5[unit_price]','$sum_total','$disunit','$Fproduct_type','1','$Fcus_back')";
                     if ($conn->query($sqlx) === TRUE) {
                     }

                  $sqlx2 = "INSERT INTO order_details (order_id,ptype_id,product_id,qty,unit_price,total_price,status_button,emp_id,status_chk_stock,face1_stock_out,face2_stock_out,qty_out,updates,status_new)
                    VALUES ('$Forder_id','$Fproduct_type','$Fproductx','$sum_qty','$row5[unit_price]','$sum_total','1','$emp_id','CB2','$Fqty','$Fqty2','$sum_qty','1','1')";
                
                if ($conn->query($sqlx2) === TRUE) { ?>
                    <script>
                        $(document).ready(function() {
                            showAlert("บันทึกข้อมูลสำเร็จyyyy", "alert-success");
                        });
                    </script>
        <?php   }
