                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="table-responsive">
                                                <table class="table text-center" id="user_table">
                                                    ...
                                                    <tbody>
                                                        <?php $sql4 = "SELECT DATE_FORMAT(po_enddate,'%Y-%m-%d') As MyDate,SUM(production_detail.qty) AS qty ,production_detail.product_id AS product_id,SUM(production_detail.a_type) AS a_type ,SUM(production_detail.b_type) AS b_type ,SUM(product.unit_price)AS unit_price,SUM(qty*unit_price) AS sumall ,SUM(a_type*unit_price) AS sum_atype ,SUM(b_type*unit_price) AS sum_btype   FROM production_detail  INNER JOIN  product  ON product.product_id=production_detail.product_id  
                                                                                    INNER JOIN  production_order ON  MONTH(production_order.po_enddate) = '$d[1]' AND YEAR(production_order.po_enddate) = '$d[0]' AND production_order.po_id=production_detail.po_id AND production_detail.status_stock='1'  GROUP BY MyDate ORDER BY MyDate  DESC  ";
